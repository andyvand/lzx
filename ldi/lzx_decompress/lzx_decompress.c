    ...
    )
{
    ...
    LDI_CONTEXT_HANDLE Handle = (LDI_CONTEXT_HANDLE)NULL;
    PVOID Source = NULL;
    PVOID Dest = NULL;
    UINT DestSize;
    ...
    if (LDIRet != MDI_ERROR_NO_ERROR)
    {
        ...
        Handle = (LDI_CONTEXT_HANDLE)NULL;
        goto Cleanup;
    }
    ...
    while (BytesRemaining > 0) {
        ...
        fprintf(stderr, "Decompressing %hu bytes to %hu bytes (source size: %ld, dest size: %ld)\n", Block.CompressedSize, Block.UncompressedSize, (ULONG)SourceSize, (ULONG)DestSize);
        ...
        if (LDIRet != MDI_ERROR_NO_ERROR)
        {
            fprintf(stderr, "Decompression failed %d, (bytes decompressed: %ld)\n", LDIRet, (ULONG)BytesDecompressed);
            goto Cleanup;
        }
...
Cleanup:
    ...
    if (Handle != (LDI_CONTEXT_HANDLE)NULL)
    {
